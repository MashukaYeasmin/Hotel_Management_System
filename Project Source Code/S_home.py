...
def Room_details():
    root.destroy()
    call(["python", "S_room.py"])

def Meal_details():
    root.destroy()
    call(["python", "S_meal.py"])
...
img2 = ImageTk.PhotoImage(Image.open("i01.jpg"))
canv.create_image(252, 0, anchor=NW, image=img2)

img3 = ImageTk.PhotoImage(Image.open("i04.jpg"))
canv.create_image(0, 240, anchor=NW, image=img3)

lbl_menu = Label(main_frame, text="MENU", font=("times new roman", 20, "bold"), bg="black", fg="gold", bd=4, relief=RIDGE)
lbl_menu.place(x=0, y=0, width=250)

btn_frame = Frame(main_frame, bd=1, relief=RIDGE)
btn_frame.place(x=0, y=40, width=250, height=200)
...
room_btn = Button(btn_frame, width=20, text="Room Details", command=Room_details, font=("times new roman", 15, "bold"), bg="black", fg="white")
room_btn.grid(row=1, column=0)

meal_btn = Button(btn_frame, width=20, text="Meal Details", command=Meal_details, font=("times new roman", 15, "bold"), bg="black", fg="white")
meal_btn.grid(row=2, column=0)

bill_btn = Button(btn_frame, width=20, text="Pay Bill", command=Bill, font=("times new roman", 15, "bold"), bg="black", fg="white")
bill_btn.grid(row=3, column=0)
